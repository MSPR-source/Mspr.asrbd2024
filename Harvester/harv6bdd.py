...
# Nmap pour l'analyse des réseaux
import nmap                          # Librairie utilisée pour le scan réseau
...
# Maintenant, importe les modèles
...
class NetworkScanner:
    ...
    def __init__(self, network_range="192.168.193.0/24"):
        """
    ...
    def scan(self, target=None):
        ...
        try:
            print(f"Scanning network: {target}...")  # Affiche l'information sur le scan
            self.scanner.scan(hosts=target, arguments='-p 1-1024 -sV -Pn -O --osscan-guess')  # Effectue le scan Nmap avec des options spécifiques

            # Vérifie si des hôtes sont détectés
            if not self.scanner.all_hosts():
                print("Aucun hôte détecté.")  # Aucun hôte trouvé
            else:
                print(f"Hôtes détectés : {', '.join(self.scanner.all_hosts())}")  # Affiche les hôtes détectés

            print("Scan terminé.")  # Indique la fin du scan
            return self.scanner  # Retourne les résultats du scan

        except Exception as e:
            print(f"Erreur pendant le scan : {e}")  # Capture et affiche l'erreur du scan
            messagebox.showerror("Erreur", f"Une erreur est survenue pendant le scan : {e}")  # Affiche un message d'erreur
            return None
...
class App:
    ...
    def create_widgets(self):
        ...
        self.consent_var = tk.BooleanVar(value=False)  # Variable pour suivre l'état de la case à cocher
        consent_checkbox = tk.Checkbutton(
            ...
            bg="#000000",
            fg="white",
            variable=self.consent_var,  # L'état de la case est stocké dans consent_var
            onvalue=True,
            offvalue=False
        )
        consent_checkbox.grid(row=6, column=0, pady=10, sticky="w")
    ...
    def login(self):
        ...
        if role == 'client':
            ...
            self.root.destroy()
            self.dashboard(username)  # Ouvre le tableau de bord avec le nom d'utilisateur
